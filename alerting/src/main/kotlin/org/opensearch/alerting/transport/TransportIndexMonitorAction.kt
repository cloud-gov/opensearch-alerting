...
class TransportIndexMonitorAction
@Inject
constructor(
    ...
    override fun doExecute(
        ...
        val transformedRequest =
                request as? IndexMonitorRequest
    ...
    /**
     * Check if user has permissions to read the configured indices on the monitor and then create
     * monitor.
     */
    fun checkIndicesAndExecute(
        ...
        searchInputs.forEach {
            val inputIndices =
                    ...
                    else if (it.name() == DOC_LEVEL_INPUT_FIELD)
                            (it as DocLevelMonitorInput).indices
        ...
        val updatedIndices =
                indices.map { index ->
                    if (IndexUtils.isAlias(index, clusterService.state()) ||
                        ...
                        val metadata =
                                clusterService.state().metadata.indicesLookup[index]?.writeIndex
        ...
        val searchRequest =
                SearchRequest()
                        .indices(*updatedIndices.toTypedArray())
                        .source(
        ...
        client.search(
                ...
                object : ActionListener<SearchResponse> {
                    override fun onResponse(searchResponse: SearchResponse) {
                        // User has read access to configured indices in the monitor, now create
                        // monitor with out user context.
                        client.threadPool().threadContext.stashContext().use {
                            IndexMonitorHandler(client, actionListener, request, user)
                                    .resolveUserAndStart()
                        }
                    }

                    //  Due to below issue with security plugin, we get security_exception when
                    // invalid index name is mentioned.
     ...
     * anomaly detector if they need to create AD monitor. As anomaly detector index is system
     * index, common user has no permission to query. So we need to send REST API call to AD REST
     * API.
     */
    ...
    inner class IndexMonitorHandler(
        ...
        fun resolveUserAndStartForAD() {
            if (user == null) {
                ...
                try {
                    ...
                    val searchRequest =
                            SearchRequest()
                                    .indices(".opendistro-anomaly-detectors")
                                ...
                                override fun onResponse(response: SearchResponse?) {
                                    ...
                                    if (totalHits != null && totalHits > 0L) {
                                        ...
                                        actionListener.onFailure(
                                                AlertingException.wrap(
                                                        OpenSearchStatusException(
                                                                "User has no available detectors",
        ...
        /**
         * This function prepares for indexing a new monitor. If this is an update request we can
         * simply update the monitor. Otherwise we first check to see how many monitors already
        ...
        private fun validateActionThrottle(
                monitor: Monitor,
                maxValue: TimeValue,
                minValue: TimeValue
        ) {
        ...
        /**
         * After searching for all existing monitors we validate the system can support another
        ...
        private fun onUpdateMappingsResponse(response: AcknowledgedResponse) {
            if (response.isAcknowledged) {
                ...
                log.info(
                        "Update ${ScheduledJob.SCHEDULED_JOBS_INDEX} mappings call not acknowledged."
                )
                actionListener.onFailure(
                        AlertingException.wrap(
                                OpenSearchStatusException(
        ...
        private suspend fun indexMonitor() {
            if (user != null) {
                // Use the backend roles which is an intersection of the requested backend roles and
                // the user's backend roles.
                // Admins can pass in any backend role. Also if no backend role is passed in, all
                // the user's backend roles are used.
                val rbacRoles =
            ...
            try {
                ...
                try { // delete monitor if metadata creation fails, log the right error and re-throw
                    // the error to fail listener
                    ...
                    log.error(
                            "failed to create metadata for monitor ${indexResponse.id}. deleting monitor"
                    )
                ...
                try {
                    if (request.monitor.isMonitorOfStandardType() &&
                                    Monitor.MonitorType.valueOf(
                                            request.monitor.monitorType.uppercase(Locale.ROOT)
                    ...
                    log.error(
                            "failed to index doc level queries monitor ${indexResponse.id}. deleting monitor",
                            t
                    )
                    cleanupMonitorAfterPartialFailure(request.monitor, indexResponse)
                    throw t
        ...
        private suspend fun cleanupMonitorAfterPartialFailure(
                monitor: Monitor,
                indexMonitorResponse: IndexResponse
        ) {
            // we simply log the success (debug log) or failure (error log) when we try clean up
            // partially failed monitor creation request
            try {
                DeleteMonitorService.deleteMonitor(monitor = monitor, RefreshPolicy.IMMEDIATE)
                log.debug(
                ...
                log.error(
                        "Failed to clean up monitor after monitor creation request partial failure",
                        e
                )
            }
        }
            ...
            try {
                ...
                if (!getResponse.isExists) {
                    actionListener.onFailure(
                            AlertingException.wrap(
                                    OpenSearchStatusException(
                                            "Monitor with ${request.monitorId} is not found",
                                            RestStatus.NOT_FOUND
                                    )
                            )
                    )
        ...
        private suspend fun onGetResponse(currentMonitor: Monitor) {
            ...
            /**
             * On update monitor check which backend roles to associate to the monitor. Below are 2
             * examples of how the logic works
             *
             ...
             * and I make a request to update the Monitor's backend roles to [a, b]. This would mean
             * that the roles to remove are [c] and the roles to add are [a, b]. The Monitor's
             ...
             * I make a request to update the Monitor's backend roles to [a, b]. This would mean
             * that the roles to remove are [c, d] and the roles to add are [a, b]. The Monitor's
            ...
            request.monitor =
                    request.monitor.copy(schemaVersion = IndexUtils.scheduledJobIndexSchemaVersion)
            val indexRequest =
                    IndexRequest(SCHEDULED_JOBS_INDEX)
                            .setRefreshPolicy(request.refreshPolicy)
                            .source(
            ...
            try {
                ...
                // Force re-creation of last run context if monitor is of type standard
                // doc-level/threat-intel
                // And monitor is re-enabled
                if (request.monitor.enabled &&
                                !currentMonitor.enabled &&
                                request.monitor.monitorType.endsWith(
                                        Monitor.MonitorType.DOC_LEVEL_MONITOR.value
                                )
                ) {
                ...
                if (!created &&
                                ...
                                Monitor.MonitorType.valueOf(
                                        currentMonitor.monitorType.uppercase(Locale.ROOT)
                                ) == Monitor.MonitorType.DOC_LEVEL_MONITOR
                ) {
                    updatedMetadata =
